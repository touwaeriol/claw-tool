...
**Key settings:**
SettingWhy`--bind lan`Binds to `0.0.0.0` so Fly’s proxy can reach the gateway`--allow-unconfigured`Starts without a config file (you’ll create one after)`internal_port = 3000`Must match `--port 3000` (or `OPENCLAW_GATEWAY_PORT`) for Fly health checks`memory = "2048mb"`512MB is too small; 2GB recommended`OPENCLAW_STATE_DIR = "/data"`Persists state on the volume
...
AspectPublicPrivateInternet scannersDiscoverableHiddenDirect attacksPossibleBlockedControl UI accessBrowserProxy/VPNWebhook deliveryDirectVia tunnel
...
[VPS Hosting](/vps)[Hetzner](/install/hetzner)
⌘I