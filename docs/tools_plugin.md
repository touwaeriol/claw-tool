...
- Apply `prependContext` to the user prompt.

- Apply `systemPrompt` override when provided.

- Apply `prependSystemContext + current system prompt + appendSystemContext`.
...
- `configureInteractive` (if present)

- `configureWhenConfigured` (only when channel status is already configured)
...
[ClawHub](/tools/clawhub)[Community plugins](/plugins/community)
⌘I