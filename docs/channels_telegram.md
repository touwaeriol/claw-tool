...
Runtime note: if `channels.telegram` is completely missing, runtime defaults to fail-closed `groupPolicy="allowlist"` unless `channels.defaults.groupPolicy` is explicitly set.Example: allow any member in one specific group:Copy
...
Example: allow only specific users inside one specific group:Copy
...
Runtime sends use the active config/secrets snapshot (startup/reload), so action paths do not perform ad-hoc SecretRef re-resolution per send.Reaction removal semantics: [/tools/reactions](/tools/reactions)
...
`agentId` is topic-only and does not inherit from group defaults.**Per-topic agent routing**: Each topic can route to a different agent by setting `agentId` in the topic config. This gives each topic its own isolated workspace, memory, and session. Example:Copy
...
Telegram distinguishes video files vs video notes.Message action example:Copy
...
Telegram polls use `openclaw message poll` and support forum topics:Copy
