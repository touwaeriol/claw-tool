...
- Action `id` must be **alphanumeric only** (`[a-zA-Z0-9]`). Hyphens and underscores break
Mattermost’s server-side action routing (returns 404). Strip them before use.

- `context.action_id` must match the button’s `id` so the confirmation message shows the
...
- Serialize with **sorted keys** and **no spaces** (the gateway uses `JSON.stringify`
with sorted keys, which produces compact output).

- Sign: `HMAC-SHA256(key=secret, data=serializedContext)`
