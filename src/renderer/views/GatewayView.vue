...
watch(
  ...
  (gw) => {
    ...
    form.value.corsOrigins = Array.isArray(gw.corsOrigins) ? gw.corsOrigins.join(', ') : (gw.corsOrigins || '')
  },
  { immediate: true }
)
...
function handleSave() {
  ...
  if (form.value.corsOrigins) {
    cfg.corsOrigins = form.value.corsOrigins.split(/[,，]/).map((s) => s.trim()).filter(Boolean)
  }
        ...
        <el-card shadow="never" class="config-card">
          ...
          <el-form :model="form" label-position="top">
            ...
            <el-form-item label="绑定模式">
              <el-select v-model="form.bind" style="width: 100%">
                ...
                >
                  <div>
                    <div>{{ mode.label }}</div>
                    <div style="font-size: 12px; color: var(--el-text-color-secondary)">{{ mode.desc }}</div>
                  </div>
                </el-option>
              </el-select>
            </el-form-item>
            ...
            <el-form-item label="CORS 允许的域名">
              <el-input v-model="form.corsOrigins" placeholder="逗号分隔，例如 http://localhost:3000" />
