<script setup>
  ...
  if (appUpdater?.getUpdateSettings) {
    ...
    if (saved.useProxyForUpdate !== undefined)
      updateSettings.value.useProxyForUpdate = saved.useProxyForUpdate
  }

  // 监听更新设置变更，同步到后端
  watch(
    () => [
      updateSettings.value.autoCheckApp,
      updateSettings.value.checkFrequency,
      updateSettings.value.useProxyForUpdate,
    ],
    ([autoCheck, freq, useProxy]) => {
      if (appUpdater?.updateSettings) {
        appUpdater.updateSettings({
          autoCheckApp: autoCheck,
          checkFrequency: freq,
          useProxyForUpdate: useProxy,
        })
      }
    },
  )
  ...
  function copyToken() {
    ...
    navigator.clipboard
      .writeText(settings.value.remoteTestToken)
      .then(() => {
  ...
  function copyAccessUrl() {
    const host =
      settings.value.remoteTestBind === '0.0.0.0' ? 'localhost' : settings.value.remoteTestBind
    ...
    <el-tabs v-model="activeTab" class="settings-tabs">
        ...
        <el-form label-position="left" label-width="160px" class="tab-form">
          <el-form-item :label="$t('settings.theme')">
            <el-radio-group v-model="settings.theme" @change="onThemeChange">
              <el-radio-button v-for="th in themes" :key="th.value" :value="th.value">{{
                $t(th.labelKey)
              }}</el-radio-button>
            </el-radio-group>
          </el-form-item>
        ...
        <el-form label-position="left" label-width="160px" class="tab-form">
          ...
          <el-form-item>
            <div class="action-row">
              <el-button type="primary" size="small" @click="saveProxyConfig">{{
              ...
              <el-button size="small" text type="warning" @click="resetProxyConfig">{{
                $t('common.reset')
              }}</el-button>
            </div>
          </el-form-item>
        ...
        <el-form label-position="left" label-width="200px" class="tab-form">
          ...
          <el-form-item :label="$t('settings.useProxyForUpdate')">
            ...
            <span style="margin-left: 8px; font-size: 12px; color: #909399">{{
              $t('settings.useProxyForUpdateTip')
            }}</span>
          </el-form-item>
          <el-form-item>
            <div class="action-row">
              <el-button :loading="appUpdateChecking" @click="checkUpdate">{{
                $t('settings.manualCheckApp')
              }}</el-button>
            </div>
          </el-form-item>
              ...
              <el-alert type="warning" :closable="false" show-icon style="width: 100%">
                <template #title>
                  ...
                  <span
                    ...
                    style="font-weight: normal; margin-left: 8px; font-size: 12px"
                  >
                ...
                <template #default>
                  ...
                  <div style="margin-top: 8px; display: flex; gap: 8px">
                    ...
                    <el-button size="small" @click="skipThisVersion">{{
      ...
      <el-tab-pane :label="$t('settings.logConfig')" name="log">
        <el-form label-position="left" label-width="160px" class="tab-form">
          ...
          <el-form-item>
            <el-button type="primary" size="small" @click="saveLogConfig">{{
              $t('settings.saveLogConfig')
            }}</el-button>
          </el-form-item>
        </el-form>
      </el-tab-pane>
      ...
      <el-tab-pane :label="$t('settings.remoteTest')" name="remote">
        <el-form label-position="left" label-width="160px" class="tab-form">
          <el-form-item :label="$t('settings.enableRemoteTest')">
            <el-switch v-model="settings.remoteTestEnabled" @change="toggleRemoteTest" />
            <el-tag
          ...
          <el-form-item :label="$t('settings.bindAddress')">
            <el-select
              v-model="settings.remoteTestBind"
              :disabled="!settings.remoteTestEnabled"
              style="width: 200px"
            >
          ...
          <el-form-item :label="$t('settings.authToken')">
            <div class="token-field">
              ...
              <el-button size="small" :loading="regeneratingToken" @click="handleRegenerateToken">{{
          ...
          <el-form-item v-if="httpServerRunning" :label="$t('settings.accessUrl')">
            <div class="token-field">
              <el-input
                ...
                style="width: 400px"
              />
              <el-button size="small" @click="copyAccessUrl">{{
                $t('settings.copyFullUrl')
              }}</el-button>
            </div>
          </el-form-item>
        </el-form>
      ...
      <el-tab-pane :label="$t('settings.configFile')" name="config">
        <div class="info-list">
          <div class="info-item">
            ...
            <span class="info-value mono-text">{{
              configStore.filePath || $t('settings.notLoaded')
            }}</span>
          </div>
          <div class="info-item">
      ...
      <el-tab-pane :label="$t('settings.about')" name="about">
        ...
        <div class="about-actions">
          ...
          <el-button text type="warning" @click="handleReset">{{
