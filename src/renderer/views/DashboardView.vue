...
async function refreshHealthCheck(executor) {
  ...
  healthItems.value[2] = {
    ...
    detail: serviceStore.gatewayRunning
      ? (gatewayReachable.value ? `端口 ${serviceStore.gatewayPort} 可达` : '运行中但端口不可达')
      : '未运行',
  }
    ...
    <div class="status-cards">
      ...
      <el-card class="status-card hover-card" shadow="never">
        <div class="card-header">
          ...
          <el-tag v-if="hasUpdate" type="warning" size="small" effect="plain" style="margin-left: auto">
            有更新
          </el-tag>
        </div>
        ...
        <div class="card-footer">
          <template v-if="hasUpdate && latestVersion">
            最新: {{ latestVersion }}
            <el-button type="primary" size="small" link :loading="updating" @click="performOpenclawUpdate" style="margin-left: 8px">
              立即更新
            </el-button>
          </template>
          <template v-else>
            实例: {{ currentInstanceName }}
            <el-button size="small" link :loading="updateChecking" @click="checkOpenclawUpdate" style="margin-left: 8px">
              检查更新
            </el-button>
          </template>
        </div>
      ...
      <el-card class="status-card hover-card" shadow="never">
        ...
        <div class="card-footer">{{ serviceStore.activeChannels.length > 0 ? serviceStore.activeChannels.join(', ') : '无活跃通道' }}</div>
      </el-card>
    </div>
    ...
    <div class="dashboard-middle">
      <el-card class="quick-actions" shadow="never">
        ...
        <div class="action-buttons">
          <el-button :type="serviceStore.gatewayRunning ? 'danger' : 'primary'" size="large" @click="toggleService">
            <el-icon>
      ...
      <el-card class="health-check" shadow="never">
        ...
        <div class="health-list">
          <div v-for="item in healthItems" :key="item.label" class="health-item">
            <el-icon
              :size="16"
              :color="item.status === 'ok' ? 'var(--ct-success)' : item.status === 'warn' ? 'var(--ct-warning)' : item.status === 'error' ? 'var(--ct-danger)' : 'var(--ct-info)'"
            >
      ...
      <div v-if="providerList.length > 0" class="provider-list">
        <div v-for="provider in providerList" :key="provider.name" class="provider-group">
          ...
          <div v-if="provider.models.length > 0" class="model-list">
            <el-table :data="provider.models" size="small" :show-header="true" stripe
              :header-cell-style="{ background: 'var(--ct-bg-base)', color: 'var(--ct-text-secondary)' }"
