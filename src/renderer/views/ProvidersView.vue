<script setup>
  ...
  const PROVIDER_PRESETS = [
    ...
    // Anthropic
    {
      ...
      defaultModels: ['claude-sonnet-4-5-20250514', 'claude-haiku-3-5-20241022'],
    },
    // Google
    {
      id: 'google-gemini',
    ...
    // GitHub
    {
      ...
      defaultModels: ['gpt-4o', 'claude-sonnet-4-5'],
    },
    // AWS
    {
      id: 'aws-bedrock',
      ...
      defaultBaseUrl: '',
      authMethod: 'aws-sdk',
      defaultModels: ['anthropic.claude-sonnet-4-5-20250514-v1:0'],
    },
    // 本地模型
    {
      id: 'ollama',
  ...
  function guessPresetId(entry) {
    const match = PROVIDER_PRESETS.find((p) => p.name === entry.key || p.id === entry.key)
...
<template>
  <div class="providers-view">
    ...
    <div class="provider-list">
      <div v-for="entry in providerEntries" :key="entry.key" class="provider-item">
        <!-- 主行 -->
        <div class="provider-header">
          <!-- 供应商图标 -->
          <div
            class="provider-avatar"
            :class="'avatar-' + getProviderIcon(entry.api || entry.apiType)"
          >
            <!-- OpenAI -->
            <svg
              v-if="getProviderIcon(entry.api || entry.apiType) === 'openai'"
              viewBox="0 0 24 24"
              width="22"
              height="22"
              fill="currentColor"
            ...
            <!-- Anthropic / Claude -->
            <svg
              v-else-if="getProviderIcon(entry.api || entry.apiType) === 'anthropic'"
              viewBox="0 0 24 24"
              width="22"
              height="22"
              fill="currentColor"
            >
              <path
                d="M17.304 3.541h-3.483l6.196 16.918h3.483L17.304 3.541zm-10.608 0L.5 20.459h3.544l1.26-3.474h6.392l1.26 3.474h3.544L10.304 3.541H6.696zm-.342 10.651L8.5 8.269l2.146 5.923H6.354z"
              />
            </svg>
            <!-- Google Gemini -->
            <svg
              v-else-if="getProviderIcon(entry.api || entry.apiType) === 'gemini'"
              viewBox="0 0 24 24"
              width="22"
              height="22"
              fill="currentColor"
            ...
            <!-- AWS -->
            <svg
              v-else-if="getProviderIcon(entry.api || entry.apiType) === 'aws'"
              viewBox="0 0 24 24"
              width="22"
              height="22"
              fill="currentColor"
            >
              ...
              />
              <path
                d="M21.698 16.207c-2.626 1.94-6.442 2.969-9.722 2.969-4.598 0-8.74-1.7-11.87-4.526-.247-.223-.024-.527.27-.351 3.384 1.963 7.559 3.153 11.877 3.153 2.914 0 6.114-.607 9.06-1.852.439-.2.814.287.385.607z"
              />
              <path
                d="M22.792 14.961c-.336-.43-2.23-.207-3.089-.104-.255.032-.295-.192-.063-.36 1.517-1.062 4.003-.754 4.291-.399.287.36-.08 2.826-1.493 4.003-.216.184-.423.088-.327-.151.319-.79 1.021-2.556.68-2.989z"
              />
            </svg>
            <!-- Ollama -->
            <svg
              v-else-if="getProviderIcon(entry.api || entry.apiType) === 'ollama'"
              viewBox="0 0 24 24"
              width="22"
              height="22"
              fill="currentColor"
          ...
          <div class="provider-info">
            ...
            <div class="provider-meta">
              <el-tag size="small" effect="plain">{{
                apiTypeLabel(entry.api || entry.apiType)
              }}</el-tag>
              <!-- API Key：长显示 + 复制 + 查看 -->
              <span v-if="entry.apiKey" class="apikey-bar">
                <el-icon :size="12" class="apikey-icon"><ElIconKey /></el-icon>
                <span class="apikey-text">{{
                  revealedKeys.has(entry.key) ? entry.apiKey : maskApiKey(entry.apiKey)
                }}</span>
                <el-button
                  text
                  size="small"
        ...
        <div class="provider-models-bar" @click="toggleModels(entry.key)">
          ...
          <el-icon
            :size="14"
            class="models-bar-arrow"
            :class="{ 'is-expanded': expandedProviders.has(entry.key) }"
          >
            <ElIconArrowDown />
          </el-icon>
        </div>
...
<style scoped>
  ...
  .provider-avatar.avatar-openai {
    background: linear-gradient(135deg, #10a37f, #0d8c6d);
  }
  .provider-avatar.avatar-anthropic {
    background: linear-gradient(135deg, #d4a27f, #c4886a);
  }
  .provider-avatar.avatar-gemini {
    background: linear-gradient(135deg, #4285f4, #886aea);
  }
  .provider-avatar.avatar-aws {
    background: linear-gradient(135deg, #ff9900, #e68a00);
  }
  .provider-avatar.avatar-ollama {
    background: linear-gradient(135deg, #888, #555);
  }
  .provider-avatar.avatar-default {
    background: linear-gradient(135deg, #409eff, #337ecc);
  }
  ...
  .test-result-badge.badge-success {
    color: var(--ct-success, #67c23a);
  }
  .test-result-badge.badge-error {
